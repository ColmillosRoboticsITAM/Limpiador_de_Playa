...
void setup() {
  ...
  // Configuración de canales PWM para los motores
  //ledcAttach(LMot_pwm, frecuenciaPWM, resolucionPWM);
  //ledcAttach(RMot_pwm, frecuenciaPWM, resolucionPWM);
}
...
void moveRobot(int velocidadPWM, int direccion) {
  ...
  // void ledcWrite(LMot_pwm, abs(velocidadPWM));
  // void ledcWrite(RMot_pwm, abs(velocidadPWM));
